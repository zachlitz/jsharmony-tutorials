jsh.App[modelid] = new (function(){
  ...
  this.api_getCust = function(onComplete){
    var emodelid = xmodel.namespace+'FormFrontEnd_Form_Popup_Get_Cust';
    //Execute the FormFrontEnd_Form_Popup_Get_Cust model
  ...
  this.api_saveCust = function(onComplete){
    ...
    //Execute the FormFrontEnd_Form_Popup_Update_CustSts model
    var params = {
      cust_id: _this.cust.cust_id,
      cust_sts: _this.cust.cust_sts 
    };
    XForm.prototype.XExecutePost(xmodel.namespace+'FormFrontEnd_Form_Popup_Update_CustSts', params, function (rslt) { //On Success
      if ('_success' in rslt) {
        if (onComplete) onComplete();
      }
